(ns process-control.actor
  "ProcessControlActor — the ISCO-08 3139 process control technician
  ...
  CRITICAL DESIGN NOTE: This actor supports process control technician
  BACK-OFFICE COORDINATION WORKFLOW only. It does NOT directly control
  setpoints, actuate valves, pump control, or emergency shutdown — those
  remain exclusively under licensed operators' and engineers' human authority.
  The Governor enforces this hard boundary (see `process-control.governor`).
  ...
  The unconditional invariant: the ProcessControlAdvisor can never directly
  commit a record or dispatch a robot action the ProcessControlGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [process-control.advisor :as advisor]
            [process-control.governor :as governor]
            [process-control.store :as store]))

(defn build-graph
  "Build a compiled ProcessControlActor graph. `store` implements
  `process-control.store/Store`. `advisor` implements `process-control.advisor/Advisor`
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:unit-id (:unit-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
